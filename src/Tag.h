...
public:
	...
	bool IsInt() const
	{
		return (m_uType == TAGTYPE_UINT64) || (m_uType == TAGTYPE_UINT32) ||
		       (m_uType == TAGTYPE_UINT16) || (m_uType == TAGTYPE_UINT8);
	}
...
protected:
	CTagIntSized(const wxString &name)
	: CTag(name)
	{
	}
	CTagIntSized(uint8 name)
	: CTag(name)
	{
	}
